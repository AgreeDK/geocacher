...
@pytest.fixture
def db_and_session(tmp_path):
    """Fresh DB + session for each test. Caller must commit/close."""
...
class TestImportLocBasic:
    def test_creates_correct_number_of_caches(self, loc_file, db_and_session):
        result = import_loc(loc_file, db_and_session)
        db_and_session.commit()
        assert result.created == 2
        assert result.total == 2

    def test_gc_codes_stored(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        codes = {c.gc_code for c in db_and_session.query(Cache).all()}
        assert "GC11111" in codes
        assert "GC22222" in codes

    def test_cache_name_stored(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        cache = db_and_session.query(Cache).filter_by(gc_code="GC11111").first()
        assert cache.name == "Forest Cache"

    def test_coordinates_stored(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        cache = db_and_session.query(Cache).filter_by(gc_code="GC11111").first()
    ...
    def test_default_cache_type_is_traditional(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        cache = db_and_session.query(Cache).filter_by(gc_code="GC11111").first()
        assert cache.cache_type == "Traditional Cache"

    def test_warning_about_limited_data(self, loc_file, db_and_session):
        result = import_loc(loc_file, db_and_session)
        assert len(result.warnings) > 0
        warning_text = " ".join(result.warnings).lower()
        assert "loc" in warning_text or "gpx" in warning_text

    def test_source_file_recorded(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        cache = db_and_session.query(Cache).filter_by(gc_code="GC11111").first()
        assert cache.source_file == loc_file.name

    def test_single_waypoint(self, tmp_path, db_and_session):
        p = tmp_path / "single.loc"
        p.write_text(SINGLE_LOC, encoding="utf-8")
        result = import_loc(p, db_and_session)
        db_and_session.commit()
...
class TestImportLocEdgeCases:
    def test_invalid_xml_returns_error(self, tmp_path, db_and_session):
        p = tmp_path / "bad.loc"
    ...
    def test_missing_coord_element_skipped(self, tmp_path, db_and_session):
    ...
    def test_non_gc_id_skipped(self, tmp_path, db_and_session):
    ...
    def test_empty_loc_file(self, tmp_path, db_and_session):
...
class TestImportLocUpsert:
    def test_reimport_does_not_duplicate(self, loc_file, db_and_session):
        import_loc(loc_file, db_and_session)
        db_and_session.commit()
        result2 = import_loc(loc_file, db_and_session)
        db_and_session.commit()

        assert result2.updated == 2
        assert result2.created == 0
        total = db_and_session.query(Cache).count()
        assert total == 2

    def test_reimport_updates_name(self, tmp_path, db_and_session):
        ...
        cache = db_and_session.query(Cache).filter_by(gc_code="GC55555").first()
